...
class Trainer2(object):
    ...
    def train(self, epoch, report_func=None, logger=None, run_times=None, name_mark=None):
        ...
        for i, batch in enumerate(self.train_iter):
            ...
            if i % 10 == 10 - 1:
                if logger is not None:
                    logger.scalar_summary("%srun%s_trainloss" % (name_mark, run_times), total_stats.loss, i+(epoch-1)*len(self.train_iter))
                    logger.scalar_summary("%srun%s_train_acc" % (name_mark, run_times), total_stats.accuracy(), i+(epoch-1)*len(self.train_iter))
                    # logger.scalar_summary("run%s_trainppl" % run_times, total_stats.ppl(), i+(epoch-1)*len(self.train_iter))
