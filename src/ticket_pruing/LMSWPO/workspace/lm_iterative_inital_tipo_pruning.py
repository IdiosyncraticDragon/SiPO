...
def main():
    ...
    while run_times < total_times:
        ...
        for prune_rate in range(1,100):
           ...
           masked_model.apply_mask()
           tmp_fit = evaluate_lm(masked_model.masked_model, valid_data, corpus, TEST_BATCH_SIZE)
           print("{} \% => validation acc {}\%, validation ppl {}".format(prune_rate, tmp_fit[1]*100., tmp_fit[0]))
        ...
        if (not best_pr) or (best_pr[0] == init_threshold[0]):
            print("Not better than last iteration of pruning, stop the process.")
            break
        ...
        saved_model_name = 'initialization_ncs_pruned_model_%s_iteration%s_%s_%s_acc_cons_%s.pt' % (name_mark, run_times, Model_type, layer_group_type, str(acc_percent_prune))
        torch.save(masked_model.masked_model, SAVE_MODEL_TMP_FOLDER+saved_model_name)

        #--------------- start retraining --------------
        model_for_train = masked_model
        
        with open(SAVE_MODEL_TMP_FOLDER + saved_model_name, 'rb') as f:
        ...
        try:
            for epoch in range(1, RETRAIN_EPOCHS + 1):
                ...
                if not best_val_loss or val_loss < best_val_loss:
                    with open("{}/initialization_{}{}_iterative_retrain_model_runtime{}_epoch_{}.pt".format(SAVE_MODEL_FOLDER, name_mark, acc_percent_prune,  run_times, epoch), 'wb') as f:
        ...
        if not recovered:
            print("The situation is unexpected! ")
            exit()
        else:
    ...
    print("init accuracy of model:", acc_of_no_prune)
    print("accuracy constraint:", acc_percent_prune)
    print("init thresholds:", init_threshold)
    previous_pr = None
    best_pr = None
    ncs_std = 0.05
    while run_times < total_times:
