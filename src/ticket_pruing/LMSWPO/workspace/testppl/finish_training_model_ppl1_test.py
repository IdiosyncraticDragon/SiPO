...
after iteration 29
lr 0.1
decay every 3 epochs
epoch 1, 2, 3: lr 0.1
epoch 4, 5, 6: lr 0.05
epoch 7, 8, 9: lr 0.025
epoch 10: lr 0.0125
decay = 0.5
...
# other_GPU_IDs = [GPU_ID, GPU_ID] # devices used for individuals, one device id for one individual
other_GPU_IDs = [int(i) for i in sys.argv[3].split(".")]
print(other_GPU_IDs)
Model_type = sys.argv[4]
layer_group_type = sys.argv[5]
try:
    acc_percent_prune = float(sys.argv[6])
...
from masked_networkT1 import MaskedModel
import copy
...
logger_path = '{}/test/luongnet-5-1-1-logs'.format(HOME_PATH)
...
# weights = SAVE_MODEL_TMP_FOLDER+"retrained_model_423GPU1213_iteration5_LuongNet_time_acc_cons_1.0.pt"
weights = SAVE_MODEL_TMP_FOLDER+"retrained_model_test1GPU710_iteration13_RNNSearch_time_acc_cons_1.0.pt"


START_THRESHOLD =  [0.9043224 , 0.24349518, 0.5193397 , 0.3580139 ]
# START_THRESHOLD = [0.73801565 ,0.6009561 , 0.7909792 , 0.2910389 ]
# START_THRESHOLD = [0.7566937 , 0.39753112 ,0.77795684, 0.17445466]
...
special_name_mark = name_mark+'ncs_'+Model_type + '_'+layer_group_type+'_'+str(acc_percent_prune)+'_lr'+str(LEARNING_RATE)
...
def build_optim(model, checkpoint, opt, pretrained_leaf_dict):
    ...
    optim._setRate(opt.learning_rate)
    optim.lr_decay = opt.lr_decay
    optim.start_decay_at = opt.start_decay_at
    optim.last_ppl = None
    print("when building optimizer:")
    print("the learning rate of opt is :", optim.lr)
    print("optimizer type:", type(optim.optimizer))
    return optim
...
def train_model(model, train_data, valid_data, fields, optim, opt, num_runs, stop_acc):
    ...
    trainer = onmt.Trainer2(model, train_iter, valid_iter,
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        print("start decay at:", optim.start_decay_at)
        print("optim.lr_decay:", optim.lr_decay)

        # 1. Train for one epoch on the training set.
        train_stats = trainer.train(epoch, report_func, logger=logger, run_times=num_runs, name_mark=special_name_mark)
        ...
        if epoch >= optim.start_decay_at and epoch % 3 == 0:
            optim._setRate(optim.lr*optim.lr_decay)
...
def opt_initialize(c_point, trans_p, trans_dum_p):
    ...
    train_opt.batch_size = TRAIN_BATCH_SIZE
...
class Statistics(object):
    ...
    def output(self, epoch, batch, n_batches, start, lr):
        ...
        print(("Epoch %2d, %5d/%5d; acc: %6.2f; ppl: %6.2f; " +
               "%3.0f src tok/s; %3.0f tgt tok/s; %6.0f s elapsed; current learning rate:%6.0f") %
...
def update_checkpoint(checkpoint, ref_net, run_times, acc_constraint, t=False):
    '''
    t = False 表示 未经过重训练
    '''
    # save model 
