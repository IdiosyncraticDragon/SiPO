...
logger_path = '{}/test/lmrunsearch-logs'.format(HOME_PATH)
...
def apply_MP_on_mask(thresholds, mask_dict, orig_dict, layer2group_map_dict, sorted_group_parameters, group_name_list):
    ...
    for the_name, the_param in mask_dict.items():
        ...
        tmp_m = Variable(orig_dict[the_name].new(tmp_size).fill_(tmp_v))
...
def train(thenet, ntokens, train_data, batch_size, bptt, corpus, grad_clip, log_interval):
...
# NCS
def NCS_MP(crates, ncs_stepsize,  masked_models, valid, corpus, acc_constraint, num_runs=0):
    total_time = 0
    total_iteration = 100
    itr_count = 0
    ...
    ref_net.apply_mask()
    orignal_fit = evaluate_lm(ref_net.masked_model, valid, corpus, TEST_BATCH_SIZE)
...
def main():
    ...
    while run_times < total_times:
        ...
        #best_found, saved_model, best_masked_model = NCS_MP(init_threshold, 0.05, masked_models, valid_data, corpus, acc_percent_prune, run_times)
        ...
        print('Finsished => acc (%.4f), ppl (%.4f)' % (tmp_fit[1]*100, tmp_fit[0]))
        ...
        try:
            for epoch in range(1, RETRAIN_EPOCHS + 1):
                ...
                train(model_for_train, ntokens, train_data, TRAIN_BATCH_SIZE, SEQ_LEN, corpus, GRAD_CLIP, TRAIN_LOG_INTERVAL)
                ...
                if not best_val_loss or val_loss < best_val_loss:
                    with open("{}/model_re{}_epoch_{}.pt".format(MODEL_PATH, runtimes, epoch), 'wb') as f:
                        torch.save(model, f)
        ...
        _, saved_model = update_checkpoint(model_for_train, run_times, acc_percent_prune, t=True)
        print(time_now(), ' saving model:', saved_model)
        print("-------------print evaluation info ---------------")
        tmp_fit = evaluate_lm(model_for_train, test_data, corpus, TEST_BATCH_SIZE)
