...
name_mark = 'lm_layer_grid' # sys.argv[1]
GPU_ID = 0 #int(sys.argv[2])
Model_type = 'LM' #sys.argv[3]
layer_group_type = 'layer' #sys.argv[4]
...
print(GPU_ID)
print(Model_type)
print(layer_group_type)
print(acc_percent_prune)
...
SAVE_MODEL_FOLDER = '{}/checkpoint/language-model/prune/'.format(MODEL_PATH)
SAVE_MODEL_TMP_FOLDER ='{}/checkpoint/language-model/prune_tmp/'.format(MODEL_PATH)
...
def main():
    ...
    for prune_rate in range(1,100):
       ...
       masked_model.apply_mask()
       tmp_fit = evaluate_lm(masked_model.masked_model, valid_data, corpus, TEST_BATCH_SIZE)
       print("{}% => validation acc {:.4f}%, validation ppl {}".format(prune_rate, tmp_fit[1]*100., tmp_fit[0]))
       masked_model.apply_mask_init()
    #    train(masked_model, ntokens, train_data, TRAIN_BATCH_SIZE, SEQ_LEN, corpus, GRAD_CLIP, TRAIN_LOG_INTERVAL, prune_rate)
    ...
    print("{:f} % => validation acc {.5f}%, validation ppl {:.5f}".format(best_pr[0], best_fit[1]*100., best_fit[0]))
    print("{:.5f} % => test acc {.5f}%, test ppl {:.5f}".format(best_pr[0], test_fit[1]*100., test_fit[0]))
    print('==============================')
