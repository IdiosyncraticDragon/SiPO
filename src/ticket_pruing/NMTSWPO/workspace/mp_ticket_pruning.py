...
import copy
from layer_group1 import group_dict #group_dict1, group_dict2
...
def make_train_data_iter(train_data, opt):
    ...
    return onmt.IO.OrderedIterator(
                dataset=train_data, batch_size=opt.batch_size//2,
...
def main():
  ...
  if GPU_ID == 0 or GPU_ID == 1:
    ...
    for i in range(len(xxx)):
      ...
      if checkpoint is not None:
        real_model = (masked_model.masked_model.module
                      if isinstance(masked_model.masked_model, nn.DataParallel)
                      else masked_model.masked_model)
        real_generator = (real_model.generator.module
                          if isinstance(real_model.generator, nn.DataParallel)
                          else real_model.generator)
        model_state_dict = real_model.state_dict()
      ...
      train_opt, _, _ = opt_initialize(checkpoint, '/fl/NMTSWPO/workspace/opennmt_translate_opt.pt', '/fl/NMTSWPO/workspace/opennmt_translate_dummy_opt.pt')
    # train data loading
      ...
      model_for_train = init_train_model(checkpoint, train_opt, train_fields) # fields need data
      ...
      masked_model = MaskedModel(model_for_train, group_dict, cuda.current_device(), cuda.current_device()) # ref_model is at current_device, no copy will happen
    ...
    # init_threshold = len(masked_models[0].group_name_list)*[0.23]
    ...
    # init_thresholds = best_found # best pop found
    ...
    # translate_opt, translate_dummy_opt = translate_opt_initialize('NMTSWPO/workspace/opennmt_translate_opt.pt', 'NMTSWPO/workspace/opennmt_translate_dummy_opt.pt')
    # translator = init_translate_model(translate_opt, translate_dummy_opt)
    # del translator.model
